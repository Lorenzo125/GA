void search_rand (struct chrom *chr) {
  ...
  struct chrom pop_aus [N_CHROM_MAX];
  double dominio_aus [N_GENE_MAX][2];
  for (u=0;u<ngene;u++){ //definisco il dominio di ricerca incentrato nel cromosoma da minimizzare in modo da non fuoriuscire dai bordi
