...
class Evaluate {
public:
   static void computeCostFit(Chromosome &chr, TH1F *data, TF1 *model)
   {
   ...
   static void computeCostFit(Population &pop, TH1F *data, TF1 *model)
   {
      for (size_t i = 0; i < pop.size(); ++i) {
         computeCostFit(pop[i], data, model);
      };
   };
};
